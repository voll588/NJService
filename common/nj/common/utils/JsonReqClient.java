...
public class JsonReqClient extends AbsRestClient {
	...
	public String findAccoutInfo(String accountSid, String authToken)
		...
		try {
			...
			//构造请求URL内容
			String timestamp = UtilDate.dateToStr(new Date(),UtilDate.DATE_TIME_NO_SLASH);
	...
	public String closeClient(String accountSid, String authToken, String clientId,String appId) {
		...
		try {
			...
			EncryptUtil encryptUtil = new EncryptUtil();
			// 构造请求URL内容
			String timestamp = UtilDate.dateToStr(new Date(),
					UtilDate.DATE_TIME_NO_SLASH);// 时间戳
			String signature =getSignature(accountSid,authToken,timestamp,encryptUtil);
			String url = getStringBuffer().append("/").append(version)
					.append("/Accounts/").append(accountSid)
					.append("/dropClient")
	...
	public String callback(String accountSid, String authToken, String appId,
		...
		try {
			...
			EncryptUtil encryptUtil = new EncryptUtil();
			// 构造请求URL内容
			String timestamp = UtilDate.dateToStr(new Date(),UtilDate.DATE_TIME_NO_SLASH);// 时间戳
			String signature =getSignature(accountSid,authToken,timestamp,encryptUtil);
			String url = getStringBuffer().append("/").append(version)
					.append("/Accounts/").append(accountSid)
	...
	@Override
	public String getAddress(String location, String output, String pois) {
		// TODO Auto-generated method stub
		String result = "";
		DefaultHttpClient httpclient=getDefaultHttpClient();
		try {
			//构造请求URL内容
			String url = getbaiduStringBuffer().append("/").append("?ak=")
					.append("249E82B509ccec8a6298595e7a1a5094")
					.append("&callback=").append("renderReverse")
					.append("&location=").append(location)
					.append("&output=").append(output)
					.append("&pois=").append(pois)
					.toString();
			HttpResponse response=get("application/json", url, httpclient);
			//获取响应实体信息
			HttpEntity entity = response.getEntity();
			if (entity != null) {
				result = EntityUtils.toString(entity, "UTF-8");
	...
	public String getlocation() {
		...
		try {
			//构造请求URL内容
			String url = "http://api.map.baidu.com/geocoder/v2/?ak=249E82B509ccec8a6298595e7a1a5094&callback=renderOption&output=json&address=外滩&city=上海市";
			HttpResponse response=get("application/json", url, httpclient);
			//获取响应实体信息
			HttpEntity entity = response.getEntity();
			if (entity != null) {
